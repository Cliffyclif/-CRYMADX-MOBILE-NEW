// ISO 3166 country list with dial codes. Flag is rendered at runtime from the
// ISO-2 code via Unicode regional-indicator pairs (no PNGs to ship).
export type Country = {
  code: string  // ISO-2
  name: string
  dial: string  // E.164 country code, no leading "+"
}
...
// ISO-2 → flag emoji via Unicode regional-indicator letters.
export function flagEmoji(iso2: string): string {
  if (!iso2 || iso2.length !== 2) return ''
  const A = 0x41
  const RIA = 0x1F1E6
  const c = iso2.toUpperCase()
  return String.fromCodePoint(c.charCodeAt(0) - A + RIA, c.charCodeAt(1) - A + RIA)
}

export function findCountry(predicate: (c: Country) => boolean): Country | undefined {
