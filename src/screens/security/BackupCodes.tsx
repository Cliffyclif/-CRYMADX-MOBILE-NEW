...
const SAMPLE = ['8x4f-29ab', '3jkc-99pl', 'm2n8-gv7e', '7q4z-2ta5', 'b9k1-44rd', 'xy77-en90', 'wp3l-6q12', '5uat-h33b', 'c9x8-bbr1', '4def-z881']

export function BackupCodes() {
  const { t } = useTranslation()
  const nav = useNavigate()
  const [codes, setCodes] = useState<string[]>(SAMPLE)
  const [used] = useState(2)
  const regen = useEndpointMutation<unknown, { codes: string[]; usedCount: number }>('api.security.backup-codes')

  useEffect(() => {
    regen.mutateAsync({}).then(d => setCodes(d.codes)).catch(() => {})
