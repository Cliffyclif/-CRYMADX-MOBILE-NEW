...
export function Announcements() {
  ...
  const { data } = useEndpoint<{ items: Announcement[] }>('api.announcements.list')
      ...
      {pinned && (
        <div className="g" style={{ padding: 14, marginTop: 8, background: 'linear-gradient(135deg, rgba(27,140,62,.1), rgba(0,200,83,.04))' }}>
          ...
          <div style={{ display: 'flex', gap: 6, marginTop: 8 }}>
            <button className="btn btn-g" style={{ flex: 1, padding: 8, fontSize: 13, margin: 0 }}>{t('rewards.tryItNow')}</button>
            <button className="btn btn-o" style={{ flex: 1, padding: 8, fontSize: 13, margin: 0 }}>{t('rewards.readPost')}</button>
          </div>
        </div>
      )}
